...
class SecureLogger:
    ...
    def log_sql_execution(self, sql: str, params: Any = None, 
                         success: bool = True, duration_ms: Optional[float] = None) -> None:
        """
    ...
    def log_patient_search(self, search_type: str, criteria_count: int, 
                          results_count: int, duration_ms: Optional[float] = None) -> None:
        """
        ...
        """
        duration_str = f" ({duration_ms:.2f}ms)" if duration_ms is not None else ""
        self.info(f"PATIENT_SEARCH: {search_type} search with {criteria_count} criteria "
    ...
    def log_authentication_event(self, event_type: str, username: Optional[str] = None, 
        ...
        if username:
            ...
            if len(username) > 4:
                masked_user = f"{username[:2]}***{username[-1:]}"
            else:
                masked_user = "***"
...
def configure_secure_logging(level: int = logging.INFO, 
                           ...
                           production_mode: bool = True) -> None:
    """
