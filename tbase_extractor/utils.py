    ...
    try:
        ...
        if templates and hasattr(templates, "is_dir") and templates.is_dir():
            ...
            if os.path.isdir(templates_str):
                print(
                    f"[DEBUG utils] Found templates via resources: {templates_str}",
                    file=sys.stderr,
                )
                return templates_str
            else:
                print(
                    f"[DEBUG utils] Resources path exists but is not a directory: {templates_str}",
                    file=sys.stderr,
                )
...
def read_ids_from_csv(csv_file_path: str, id_column_name: str, logger: logging.Logger) -> List[str]:
    ...
    try:
        with open(
            ...
            encoding="utf-8-sig",
            newline="",
        ) as infile:  # utf-8-sig for BOM
            reader = csv.DictReader(infile)
            if not reader.fieldnames:
                logger.error(
                    f"CSV file '{csv_file_path}' appears to be empty or improperly formatted.",
                )
                return ids

            if id_column_name not in reader.fieldnames:
                logger.error(
                    f"ID column '{id_column_name}' not found in CSV header. Available columns: {reader.fieldnames}",
                )
                return ids
            ...
            for row_num, row in enumerate(reader, 1):
                ...
                if patient_id_str and patient_id_str.strip():
                    ...
                    logger.warning(
                        f"Missing or empty ID in CSV file '{csv_file_path}' at row {row_num}.",
                    )
    ...
    if not ids:
        logger.warning(
            f"No IDs extracted from CSV file '{csv_file_path}' with ID column '{id_column_name}'.",
        )
    else:
