...
from .config import (
    APP_VERSION,
    METADATA_PARAM_KEYS,
    STATUS_BATCH_INPUT_ERROR,
    STATUS_BATCH_NO_DATA,
    STATUS_BATCH_PARTIAL_SUCCESS,
    STATUS_BATCH_SUCCESS_ALL,
    STATUS_SUCCESS,
    STATUS_SUCCESS_NO_DATA,
)
...
def extract_query_parameters(args: Any) -> Dict[str, str]:
    ...
    return {k: str(v) for k, v in vars(args).items() if k in METADATA_PARAM_KEYS and v is not None}
