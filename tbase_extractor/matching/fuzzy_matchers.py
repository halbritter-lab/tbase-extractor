"""Fuzzy matching utilities for patient data comparison."""
...
class FuzzyMatcher:
    """Handles fuzzy matching for patient name and date comparisons."""
    ...
    def __init__(
        ...
        """Initialize fuzzy matcher with similarity thresholds."""
    ...
    def compare_names(
        ...
        """Compare two names and return match information."""
        ...
        if not input_name_clean:  # Input name not provided or empty
            return MatchInfo(
                ...
                db_name,
                "NotCompared",
                details="Input name not provided",
            )
    ...
    def compare_dates(self, input_dob: Optional[date], db_dob: Optional[date]) -> MatchInfo:
        """Compare two dates and return match information."""
        ...
        if input_dob is None:
            return MatchInfo(
                ...
                db_dob,
                "NotCompared",
                details="Input DOB not provided",
            )
        if db_dob is None:
            return MatchInfo(
                _FIELD_NAME_DOB,
