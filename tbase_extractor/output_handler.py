...
def process_match_candidates_for_tabular(results_envelope: List[Any]) -> List[Dict[str, Any]]:
    ...
    if not (
        isinstance(results_envelope, list)
        and results_envelope
        and hasattr(results_envelope[0], "match_fields_info")
        and hasattr(results_envelope[0], "overall_score")
    ):
...
def handle_split_output(
    ...
    for group_key, patient_rows in grouped_data.items():
        try:
            ...
            files_saved += 1
            logger.debug(
                f"Saved {len(patient_rows)} rows for patient group '{group_key}' to {row_file_path}",
            )
    ...
    try:
        if output_file_path:
            if split_output and processed_results:
                ...
                output_dir = os.path.dirname(output_file_path) or os.getcwd()
                logger.info(
                    f"Saved {files_saved} individual files for '{query_display_name}' in {output_dir}",
                )
            else:
                write_output_to_file(
                    output_file_path,
