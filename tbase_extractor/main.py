...
from .utils import read_ids_from_csv, resolve_templates_dir
...
def parse_dob_str(dob_str: Optional[str], logger: logging.Logger) -> Optional[date]:
    ...
    try:
        ...
        logger.error(
            f"Invalid Date of Birth format for '{dob_str}'. Please use '{DOB_FORMAT}' (e.g., 1990-12-31).",
        )
    ...
    if output_file_path:
        ...
        if ext == ".json":
            ...
            if ext:
                ...
                logger.warning(
                    f"No output file extension provided for '{output_file_path}'. " f"Defaulting to 'json' format.",
                )
            return "json"
...
def setup_arg_parser():
    ...
    parser_list_tables = subparsers.add_parser(
        "list-tables",
        help="List all available base tables in the database.",
    )
    ...
    parser_query.add_argument(
        ...
        default=None,
        help="Output format: json, csv, tsv, txt, or stdout (pretty table to console). "
        "Inferred from -o extension if not set.",
    )
    parser_query.add_argument(
        "--optimize-txt",
        action="store_true",
        help="When using TXT format, optimize output by showing patient info once "
        "followed by varying data (like diagnoses).",
    )
    ...
    if results is not None:  # results can be an empty list
        ...
        if hasattr(args, "batch_info") and args.batch_info:
            ...
            if results:
                ...
                total_input = args.batch_info.get(
                    "total_ids_in_csv",
                    args.batch_info.get("total_rows_in_csv", 0),
                )
                if total_input > 0:
                    metadata_dict["status"] = "batch_processed_no_data_or_all_failed"
                else:
...
def handle_list_tables(
    ...
    use_dynamic = getattr(args, "use_dynamic_builder", False)
    if (
        hasattr(query_manager, "get_list_tables_query")
        and hasattr(query_manager.get_list_tables_query, "__code__")
        and "use_dynamic" in query_manager.get_list_tables_query.__code__.co_varnames
    ):
...
def handle_get_patient_by_id(
    ...
    if args.input_csv:
        ...
        if args.patient_id is not None:
            ...
            parser.error(
                "Cannot use --patient-id (-i) and --input-csv (-ic) simultaneously for query 'get_patient_by_id'.",
            )
        ...
        for id_str in patient_id_strings:
            ...
            if (
                hasattr(query_manager, "get_patient_by_id_query")
                and "include_diagnoses" in query_manager.get_patient_by_id_query.__code__.co_varnames
            ):
                sql, params = query_manager.get_patient_by_id_query(
                    current_patient_id,
            ...
            if db.execute_query(sql, params):
                ...
                if fetched_data is not None:
                    ...
                    if not fetched_data:
                        logger.info(
                            f"Query for Patient ID {current_patient_id} (from CSV) returned no data.",
                        )
                else:
                    logger.error(
                        f"Error fetching results for Patient ID {current_patient_id} (from CSV).",
                    )
                    failed_ids_details[str(current_patient_id)] = "Fetch error"  # Use str for key consistency
            else:
                logger.error(
                    f"Query execution failed for Patient ID {current_patient_id} (from CSV).",
                )
        ...
        if args.patient_id is None:
            logger.error(
                "Argument --patient-id/-i is REQUIRED for query 'get_patient_by_id' (or use --input-csv).",
            )
            parser.error(
                "Argument --patient-id/-i is REQUIRED for query 'get_patient_by_id' (or use --input-csv).",
            )
        ...
        if db.execute_query(sql, params):
            ...
            if fetched_data is not None:
                if not fetched_data:
                    logger.info(
                        f"Query executed successfully, but no data found for Patient ID {args.patient_id}.",
                    )
...
def handle_patient_by_name_dob(
    args: argparse.Namespace,
    query_manager: QueryManager,
    db: SQLInterface,
    logger: logging.Logger,
    ...
    if not all([args.first_name, args.last_name, args.dob]):
        logger.error(
            "Arguments --first-name/-fn, --last-name/-ln, and --dob/-d are REQUIRED for query 'patient-by-name-dob'.",
        )
        parser.error(
            "Arguments --first-name/-fn, --last-name/-ln, and --dob/-d are REQUIRED for query 'patient-by-name-dob'.",
        )
    ...
    try:
        ...
        parser.error(
            f"Invalid Date of Birth format for --dob/-d. Please use '{DOB_FORMAT}' (e.g., 1990-12-31).",
        )

    logger.info(
        f"Attempting to execute: {query_display_name} for Name={args.first_name} {args.last_name}, DOB={args.dob}",
    )

    # Check if we're using dynamic query manager and pass include_diagnoses parameter
    include_diagnoses = getattr(args, "include_diagnoses", False)
    if (
        hasattr(query_manager, "get_patient_by_name_dob_query")
        and "include_diagnoses" in query_manager.get_patient_by_name_dob_query.__code__.co_varnames
    ):
    ...
    if db.execute_query(sql, params):
        ...
        if fetched_data is not None:
            if not fetched_data:
                logger.info(
                    f"Query executed successfully, but no data found for FirstName='{args.first_name}', LastName='{args.last_name}', DOB='{args.dob}'.",
                )
            return fetched_data, query_display_name
        else:
            logger.error("Error occurred while fetching results.")
...
def handle_patient_fuzzy_search(
    args: argparse.Namespace,
    query_manager: QueryManager,
    db: SQLInterface,
    logger: logging.Logger,
    ...
    if not any([args.first_name, args.last_name, args.dob]):
        logger.error(
            "At least one search parameter (--first-name/-fn, --last-name/-ln, or --dob/-d) is REQUIRED for fuzzy search.",
        )
    ...
    search_params = {"first_name": args.first_name, "last_name": args.last_name, "dob": dob_object}
    ...
    results = strategy.search(
        search_params,
        min_overall_score=args.min_match_score,
        include_diagnoses=include_diagnoses,
    )
...
def handle_get_table_columns(
    ...
    if not args.table_name or not args.table_schema:
        logger.error(
            "Arguments --table-name and --table-schema are REQUIRED for query 'get-table-columns'.",
        )
        parser.error(
            "Arguments --table-name and --table-schema are REQUIRED for query 'get-table-columns'.",
        )
    ...
    logger.info(
        f"Attempting to execute: get-table-columns query for {args.table_schema}.{args.table_name}",
    )
    ...
    if db.execute_query(sql, params):
        ...
        if not fetched_column_data:
            logger.info(
                f"Query executed successfully, but no columns found for {args.table_schema}.{args.table_name}.",
            )
            ...
            logger.info(
                f"Successfully retrieved {column_count} columns for {args.table_schema}.{args.table_name}",
...
def handle_discover_patient_tables(
    ...
    if db.execute_query(sql, params):
        ...
        if fetched_data is not None:
            if not fetched_data:
                logger.info(
                    "Query executed successfully, but found no tables with patient ID columns.",
                )
            else:
                table_count = len(fetched_data)
                logger.info(
                    f"Successfully discovered {table_count} tables with patient ID columns in schema '{args.schema}'",
                )
            return fetched_data, query_display_name
        else:
            logger.error("Error occurred while fetching results.")
...
def handle_query_custom_tables(
    ...
    if args.input_csv:
        ...
        if args.patient_id is not None:
            logger.error("Cannot use --patient-id (-i) and --input-csv (-ic) simultaneously.")
            parser.error(
                "Cannot use --patient-id (-i) and --input-csv (-ic) simultaneously for query-custom-tables.",
            )
        ...
        if args.patient_id is None:
            logger.error(
                "Argument --patient-id/-i is REQUIRED for query-custom-tables (or use --input-csv).",
            )
            parser.error(
                "Argument --patient-id/-i is REQUIRED for query-custom-tables (or use --input-csv).",
            )
...
def handle_batch_search_demographics(
    args: argparse.Namespace,
    query_manager: QueryManager,
    db: SQLInterface,
    logger: logging.Logger,
    ...
    """
    query_display_name = "Batch Demographic Search"
    ...
    logger.info(
        f"Attempting to execute: {query_display_name} from {os.path.basename(args.input_csv)}",
    )
    ...
    for patient in patients_data:
        ...
        try:
            ...
            logger.debug(
                f"Batch processing: Searching for patient FirstName={first_name}, LastName={last_name}, DOB={dob_str}",
            )
            ...
            if db.execute_query(sql, params):
                ...
                if fetched_data is not None:
                    if fetched_data:  # Only count as successful if data was found
                        ...
                        logger.info(
                            f"Row {row_num}: No data found for FirstName='{first_name}', LastName='{last_name}', DOB='{dob_str}'",
                        )
    ...
    # Log summary
    logger.info(
        f"Batch processing summary: Successfully processed {successful_count} out of {len(patients_data)} rows from CSV.",
    )
